...
# Script pour pousser uniquement le dossier server vers un nouveau dépôt GitHub via SSH
...
echo "Entrez l'URL SSH du nouveau dépôt GitHub (format: git@github.com:username/repo.git):"
read repo_url
...
# Vérifier si le format de l'URL est correct (SSH)
if [[ ! "$repo_url" =~ ^git@.*:.*/.*\.git$ ]]; then
    echo "Format d'URL SSH incorrect. L'URL doit être au format git@github.com:username/repo.git"
    exit 1
fi
