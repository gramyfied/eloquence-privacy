...
class _ArticulationExerciseScreenState extends State<ArticulationExerciseScreen> {
  ...
  Future<void> _initializeServicesAndText() async {
    try {
      ...
      // Essayer d'obtenir la syllabification et préparer les textes
      String? syllabifiedString = _syllabificationService.getSyllabification(_textToRead);

      if (syllabifiedString != null && syllabifiedString.isNotEmpty) {
        _displayText = syllabifiedString; // Garde les tirets pour l'affichage
        // Sépare les syllabes en utilisant le tiret comme délimiteur, en supprimant les espaces autour
        _syllables = syllabifiedString.split(RegExp(r'\s*-\s*')).map((s) => s.trim()).where((s) => s.isNotEmpty).toList();
        _referenceTextForAzure = _syllables.join(' '); // Syllabes séparées par espace pour Azure
        ConsoleLogger.info('Texte syllabifié trouvé: "$_displayText"');
        ConsoleLogger.info('Syllabes: $_syllables');
        ConsoleLogger.info('Texte référence Azure: "$_referenceTextForAzure"');
      } else {
        _displayText = _textToRead; // Afficher l'original si non trouvé
        _syllables = [_textToRead]; // Considérer le mot entier comme une seule "syllabe" pour Azure
        _referenceTextForAzure = _textToRead;
        ConsoleLogger.warning('Syllabification non trouvée pour "$_textToRead". Utilisation du texte original.');
      }
          ...
          switch (result.type) {
            ...
            case AzureSpeechEventType.finalResult:
              setState(() {
                ...
                _lastRecognizedText = result.text ?? '';
                _isProcessing = false; // Arrêter le processing car on a un résultat final

                // Extraire les détails de l'évaluation de prononciation depuis 'result'
                final pronunciationResultJson = result.pronunciationResult;
                ConsoleLogger.info('[UI] Résultat Pronunciation Assessment reçu: $pronunciationResultJson');

                // Analyser le résultat Azure et préparer les données pour OpenAI
                double overallScore = (pronunciationResultJson?['AccuracyScore'] as num?)?.toDouble() ?? 0.0;
                double pronScore = (pronunciationResultJson?['PronunciationScore'] as num?)?.toDouble() ?? 0.0;
                double fluencyScore = (pronunciationResultJson?['FluencyScore'] as num?)?.toDouble() ?? 0.0;
                List<Map<String, dynamic>> wordsDetails = []; // Pour stocker les détails par "mot" (syllabe)

                // --- Début de l'analyse détaillée (à adapter selon la structure JSON réelle) ---
                if (pronunciationResultJson != null && pronunciationResultJson['Words'] is List) {
                  final List words = pronunciationResultJson['Words'];
                  for (var wordData in words) {
                    if (wordData is Map<String, dynamic>) {
                      ...
                      if (wordData['Phonemes'] is List) {
                        for (var phonemeData in wordData['Phonemes']) {
                ...
                // Mettre à jour l'état avec un résultat préliminaire basé sur Azure
                setState(() {
                  _evaluationResult = ArticulationEvaluationResult(
                     score: overallScore,
                     syllableClarity: (pronunciationResultJson?['PronunciationScore'] as num?)?.toDouble() ?? 0.0, // Utiliser PronunciationScore comme approximation
                     ...
                     details: pronunciationResultJson // Stocker les détails bruts
                   );
                });

                ConsoleLogger.info('[UI] Résultat final Azure traité. Lancement de la génération de feedback OpenAI.');
                _getOpenAiFeedback(_evaluationResult); // Lancer la génération de feedback OpenAI
              });
  ...
  Future<void> _getOpenAiFeedback(ArticulationEvaluationResult? azureResult) async {
    ...
    // Préparer les arguments pour le service OpenAI
    final Map<String, dynamic> metrics = {
      'score_global_accuracy': azureResult.score,
      'score_prononciation': azureResult.syllableClarity, // Approximation
